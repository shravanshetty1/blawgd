...
async fn main() -> std::io::Result<()> {
    let mnemonic: &str = "voice salt fortune fork draw endless figure layer need begin trouble use cream will alpha cheese glad cook monkey used rigid better describe demise";
    let wallet = crw_wallet::crypto::MnemonicWallet::new(mnemonic, COSMOS_DP)
        .expect("could not generate alice wallet");
    let sender = State::new(wallet, "http://localhost:9090".to_string());
    println!("{}", mnemonic);
